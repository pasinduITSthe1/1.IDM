...
/// MRZ Scanner - Uses Google ML Kit + Tesseract OCR (FREE)
/// This is the working scanner from test_app_mrz
  ...
  Future<void> _captureAndScan() async {
    ...
    try {
      ...
      if (mrzData != null && mounted) {
        // Navigate to guest registration with scanned data
        context.go('/register-guest', extra: mrzData);
      } else {
        setState(() {
      ...
      for (int i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('P<') && i + 1 < lines.length) {
          ...
          if (line2.length >= 28) {
            return {
              ...
              'expiryDate': line2.length > 27 ? _formatDate(line2.substring(21, 27)) : '',
            };
          }
        }
        ...
        if ((line.startsWith('ID') || line.startsWith('AC') || line.startsWith('I<')) 
          ...
          return {
            ...
            'documentNumber': line.length > 15 ? line.substring(5, 14).replaceAll('<', '') : '',
            'nationality': '',
            'dateOfBirth': '',
            'sex': '',
            'expiryDate': '',
